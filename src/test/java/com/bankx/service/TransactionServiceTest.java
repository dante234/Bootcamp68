...
public class TransactionServiceTest {
    ...
    @BeforeEach
    void setUp() {
        transactionService = new TransactionService(accountRepository, transactionRepository, riskService, txSink, new LogContext());
    }
    ...
    void testCreate_InsufficientFunds() {
        ...
        when(accountRepository.findByNumber("12345")).thenReturn(Mono.just(account));
        when(riskService.isAllowed("USD", "DEBIT", BigDecimal.valueOf(200))).thenReturn(Mono.just(true));
    ...
    void testByAccount_Success() {
        ...
        when(accountRepository.findByNumber("12345")).thenReturn(Mono.just(account));
        when(transactionRepository.findByAccountIdOrderByTimestampDesc(account.getId())).thenReturn(Flux.just(transaction));
    ...
    void testStream() {
        ...
        transactionService = new TransactionService(accountRepository, transactionRepository, riskService, sink, new LogContext());
